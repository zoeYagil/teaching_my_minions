...
#%% THINGS TO CHANGE
file_path = '/Users/danielsinausia/Documents/Experiments/CZ_00001/PC2-15/PCA_scores.txt'


def load_file_from_new_directory():
    new_directory = '/Users/danielsinausia/Downloads/Potentiostat 2'
    for file_name in os.listdir(new_directory):
        if file_name.endswith('.csv'):
            return os.path.join(new_directory, file_name)
    
    raise FileNotFoundError("Integrated areas file not found in the new directory.")
...
window_length = 10  # SG 
polyorder = 2 #SG
peak_threshold = 5 #detect peaks with magnitude beyond this value
trough_threshold = 5#detect troughs with magnitude beyond this value
...
integrated_areas_file = load_file_from_new_directory()
integrated_areas_data = pd.read_csv(integrated_areas_file) 
integrated_time = integrated_areas_data.iloc[:, 0].values
integrated_areas_data = integrated_areas_data.drop(columns=integrated_areas_data.columns[0])
def calculate_spearman_for_discontinuities(surrounding_times, surrounding_values, integrated_time, integrated_areas_data):
    spearman_coefficients = {}
    for i, times in enumerate(surrounding_times):
        spearman_coefficients[f'Discontinuity_{i+1}'] = {}
        closest_indices = [np.where(np.abs(integrated_time - t) == np.min(np.abs(integrated_time - t)))[0][0] for t in times]
...
spearman_results = calculate_spearman_for_discontinuities(surrounding_times, surrounding_values, integrated_time, integrated_areas_data)
...
def explain_spearman_visualization(pca_values, integrated_values, discontinuity_time, output_folder):
    ...
    # Step 1: Rank the PCA and Integrated Areas values
    pca_ranks = np.argsort(pca_values) + 1
    integrated_ranks = np.argsort(integrated_values) + 1
